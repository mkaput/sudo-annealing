...
bool is_safe_to_put(uchar *my_sudoku, uchar num, size_t i) {
    return check_row(my_sudoku, num, i)
        && check_col(my_sudoku, num, i)
        && check_box(my_sudoku, num, i);
}

bool stack_push(
    size_t value,
    private size_t *stack,
    private size_t *stack_top
) {
    if (*stack_top == STACK_MAX) return false;
    *stack_top += 1;
    ...
    rec: while (true) {
        ...
        for (uchar num = my_sudoku[stack[stack_top]] + 1; num <= 9; num++) {
            if (is_safe_to_put(my_sudoku, num, stack[stack_top])) {
                ...
                if (!stack_push(stack[stack_top] + 1, stack, &stack_top)) break;
                goto rec; // recursion!
            }
        }
